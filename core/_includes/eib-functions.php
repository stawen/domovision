...
function dptSelectDecode($dpt, $data, $inverse = false) {
    $All_DPT = All_DPT();
    $type    = substr($dpt, 0, strpos($dpt, '.'));
    switch ($type) {
        case "1": //version domovision ok
			// 0:off 1:on
            //$value = ($data == 0)?1:0;
            $value = $data & 0x1;
            break;
        case "2": //version domovision ok, idem que DPT1 mais avec un priority control
            $value = 0x0;
			if(($data >> 1)){ // si le control est a 1, on met en sortie la valeur du premier bit
				$value = $data & 0x1;
			}
            break;
        case "3": //version domovision | A verifier
            $ctrl     = ($data & 0x08) >> 3;
            $stepCode = $data & 0x07;
            $value = Array(
							$ctrl,
            ...
            break;
		case "4": //version domovision OK
			 $value = chr($data);
			break;
        case "5": //version domovision OK
            switch ($dpt) {
                case "5.001":
                    $value = round((intval($data) * 100) / 255);
                    break;
                case "5.003":
                    $value = round((intval($data) * 360) / 255);
                    break;
                case "5.004":
                    $value = round(intval($data));
                    break;
                default:
                    $value = $data;
                    break;
            ...
            break;
        case "6": //version domovision OK
            if ($data >= 0x80)
                $value = -(($data - 1) ^ 0xff); # invert twos complement
            else
                $value = $data;
            break;
        case "7": //version domovision OK
            $value = $data;
            break;
        case "8": //version domovision OK
            if ($data >= 0x8000){
        ...
        case "9":  //version domovision ok
			...
			if ($data >> 15) {
				//le nombre doit etre négatif, donc on fait le complement du mantissa
				$mantissa = $mantissa ^ 0x7FF;
				$sign     = -1;
			}
			
			$value = $sign * $mantissa  * 0.01 * pow(2 , $exponent);
            break;
        case "10":
			/*
            $wDay  = ($data[0] >> 5) & 0x07;
        ...
        case "11":
            ...
            break;
        case "12": //version domovision ok
            $value = $data;
            break;
        case "13": //version domovision ok
            $value = $data;
            if ($value >= 0x80000000)
                $value = -(($value - 1) ^ 0xffffffff); # invert twos complement           
            break;
        case "14": //version domovision ok - IEEE 754
            $exp = ($data >> 23 & 0xFF) - 127;
			$mantissa = ($data & ((1 << 23) - 1)) + (1 << 23) * ($data >> 31 | 1);

			$value = $mantissa * pow(2, $exp - 23);
        ...
        case "16": //character string
            ...
            break;
        case "17": // //version domovision ok Scene
            $$ctrl   = $data >> 7;
			$scene	 = $data & 0x3F;
        ...
        case "20":
            $value = $data;
            if ($dpt != "20.xxx") {
                $value = $All_DPT["8BitEncAbsValue"][$dpt]["Valeurs"][$data];
            }
            break;
    }
    return $value;
}
// retourne l'unité du dpt
...
//retrouve le type du dpt
function recursive_array_search($needle,$haystack) {
    foreach($haystack as $key=>$value) {
        $current_key=$key;
        if(($needle===$value OR $needle===$key) OR (is_array($value) && recursive_array_search($needle,$value) !== false)) {
            return $current_key;
        }
    }
    return false;
...
//retourne la valeur formaté en fonction du dpt
