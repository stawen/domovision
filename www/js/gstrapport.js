$(document).ready(function() {
    ...
    function initModalAddGraphe(){
        $('#modal_graphique').on('show.bs.modal', function() {
	        ...
	        $.getJSON("ajax.php?action=conf&data=getLastGraphePosition", function(json) {
    ...
    function addGraphe(){
		...
		$.getJSON("ajax.php?action=conf&data=testGrapheExist&name=" + $('#modal_graphique').find('#name').val(), function(json) {
			//console.log(json);
			if (json.exist === 0) {
				//so le groupe n'existe pas, on enregistre
				$.ajax({
					url: 'ajax.php?action=conf&data=addGraphe',
					type: 'POST',
					data: $.param(tab),
					async: false,
					success: function(a) {
    ...
    function updateGraphe(){
		...
		$.getJSON("ajax.php?action=conf&data=testGrapheExist&name=" + tab.name, function(json) {
			...
			if (json.exist === 0) {
				//so le groupe n'existe pas, on enregistre
				$.ajax({
					url: 'ajax.php?action=conf&data=updateGraphe',
					type: 'POST',
					data: $.param(tab),
					async: false,
					success: function(a) {
    ...
    function deleteGraphe(){
		...
		$.ajax({
			url: 'ajax.php?action=conf&data=deleteGraphe',
			type: 'POST',
			data: $.param(tab),
			async: false,
			success: function(a) {
    ...
    function addAsso(){
		...
		//console.log(tab.position);
		//test si le groupe adrress n'est pas déja utilisé
		$.getJSON("ajax.php?action=conf&data=testAssoGrapheExist&graphe=" + $('#modal_asso').find('#select_graphe').val() + "&eqt=" + $('#modal_asso').find('#select_eqt').val(), function(json) {
			//console.log(json);
			if (json.exist === 0) {
				//so l'asso n'existe pas, on enregistre
				$.ajax({
					url: 'ajax.php?action=conf&data=addGrapheAsso',
					type: 'POST',
					data: $.param(tab),
					async: false,
					success: function(a) {
				...
				$.growlWarning("Attention, le couple Graphique + Equipement existe déjà");
			}
		});
    }
    ...
    function updateAsso(){
		...
		//test si le groupe adrress n'est pas déja utilisé
		$.getJSON("ajax.php?action=conf&data=testAssoGrapheExist&graphe=" + tab.id_graphe + "&eqt=" + tab.id_eqt, function(json) {
			//console.log(json);
			if (json.exist === 0) {
				//so l'asso n'existe pas, on enregistre
				$.ajax({
					url: 'ajax.php?action=conf&data=updateGrapheAsso',
					type: 'POST',
					data: $.param(tab),
					async: false,
					success: function(a) {
    ...
    function deleteAssoGraphe(){
		...
		$.ajax({
			url: 'ajax.php?action=conf&data=deleteAssoGraphe',
			type: 'POST',
			data: $.param(tab),
			async: false,
			success: function(a) {
       ...
       // $('#select_groupe').find('option').remove();
		...
		$.getJSON("ajax.php?action=conf&data=getGraphe", function(json) {
				...
				$.each(json, function(key, val) {
    				...
    				$('#listeGraphique > tbody:last').append('<tr>  <td> \
    ...
    function refreshTableAsso(){
		...
		$.getJSON("ajax.php?action=conf&data=getGrapheAsso&graphe=" + $('#select_graphique').val(), function(json) {
